...
export const getAllFlashcards = async (page = 1, limit = 20) => {
  const response = await api.get('/v1/flashcards', { params: { page, limit } });
  return response.data;
};

export const getFlashcard = async (cardId) => {
  const response = await api.get(`/v1/flashcards/${cardId}`);
  return response.data;
};

export const createFlashcard = async (data) => {
  const response = await api.post('/v1/flashcards', data);
  return response.data;
};

export const createFlashcardFromSession = async (sessionId) => {
  const response = await api.post('/v1/flashcards/from-session', null, {
    params: { session_id: sessionId }
  });
  return response.data;
};

export const reviewFlashcard = async (cardId, quality, timeSpent = 0) => {
  const response = await api.post(`/v1/flashcards/${cardId}/review`, { quality, time_spent: timeSpent });
  return response.data;
};

export const deleteFlashcard = async (cardId) => {
  const response = await api.delete(`/v1/flashcards/${cardId}`);
  return response.data;
};

export const getFlashcardStats = async () => {
  const response = await api.get('/v1/flashcards/stats');
  return response.data;
};
